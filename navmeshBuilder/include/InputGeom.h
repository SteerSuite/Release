...
public:
	...
	bool loadMesh(class rcContext* ctx, std::vector<Util::Point> verts, std::vector<size_t> triVerts);
	...
	void addCube(float x, float y, float length, float width)
	{
		this->m_mesh->addCube(x, y, length, width);
	}
};
