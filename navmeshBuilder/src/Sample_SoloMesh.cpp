...
bool Sample_SoloMesh::handleBuild()
{
	...
	/*
	duFileIO * fileIO = new duFileIO();
	if (m_pmesh)
	{
		 duDumpPolyMeshToObj(*m_pmesh, "pmesh.obj");
	}
	if (m_dmesh)
	{
		 duDumpPolyMeshDetailToObj(*m_dmesh, "dmesh.obj");
	}
	*/
	return true;
}
...
/**
 * Returns the mesh in almost obj format.
 * Indicies start from 0 for this data not one
 */

std::pair<std::vector<Util::Point> , std::vector<size_t>> Sample_SoloMesh::getNavMeshGeometry()
{// From RecastDump.cpp
	...
	for (int i = 0; i < m_dmesh->nverts; ++i)
	{
		...
		verts.push_back(Util::Point(v[0],v[1],v[2]));
		// ioprintf(io, "v %f %f %f\n", v[0],v[1],v[2]);
	}


	for (int i = 0; i < m_dmesh->nmeshes; ++i)
	{ // There can be submeshes for the navmesh
		...
		{ // Oh pointers...
			triVerts.push_back((int)(bverts+tris[j*4+0]));
			triVerts.push_back((int)(bverts+tris[j*4+1]));
			triVerts.push_back((int)(bverts+tris[j*4+2]));
			/*ioprintf(io, "f %d %d %d\n",
 ...
 * Indicies start from 0 for this data not one
 */
std::pair<std::vector<Util::Point> , std::vector<size_t>> Sample_SoloMesh::getEnvironemntGeometry()
{
	std::vector<Util::Point> verts;
	std::vector<size_t> triVerts;
	...
	for (int i = 0; i < m_pmesh->nverts; ++i)
	{
		...
		verts.push_back(Util::Point(x,y,z));
		// ioprintf(io, "v %f %f %f\n", x,y,z);
	}
	...
	for (int i = 0; i < m_pmesh->npolys; ++i)
	{
		...
		for (int j = 2; j < nvp; ++j)
		{
			...
			triVerts.push_back(p[0]);
			triVerts.push_back(p[j-1]);
			triVerts.push_back(p[j]);
			// ioprintf(io, "f %d %d %d\n", p[0]+1, p[j-1]+1, p[j]+1);
		}
	}
