...
bool InputGeom::loadMesh(rcContext* ctx, std::vector<Util::Point> verts, std::vector<size_t> triVerts)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
	...
	for (size_t vert=0; vert < verts.size(); vert++ )
	{
		Util::Point p = verts.at(vert);
		m_mesh->addVertex(p.x,p.y,p.z);
	}
	for (size_t trivert=0; trivert < triVerts.size(); trivert+=3 )
	{
		m_mesh->addTriangle(triVerts.at(trivert), triVerts.at(trivert+1), triVerts.at(trivert+2));
	}
