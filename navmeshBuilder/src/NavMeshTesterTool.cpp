...
NavMeshTesterTool::NavMeshTesterTool() :
	...
	m_toolMode(TOOLMODE_PATHFIND_FOLLOW), // Glen Changed tool Mode
...
float NavMeshTesterTool::getPathLength()
{
	float pathLength=0;
	for (int i = 1; i < m_nsmoothPath; ++i)
	{
		float x_2=0, y_2=0, z_2=0;
		x_2 = (m_smoothPath[(i-1)*3] - m_smoothPath[i*3]) * (m_smoothPath[(i-1)*3] - m_smoothPath[i*3]);
		y_2 = (m_smoothPath[(i-1)*3+1] - m_smoothPath[i*3+1]) * (m_smoothPath[(i-1)*3+1] - m_smoothPath[i*3+1]);
		z_2 = (m_smoothPath[(i-1)*3+2] - m_smoothPath[i*3+2]) * (m_smoothPath[(i-1)*3+2] - m_smoothPath[i*3+2]);
		pathLength+=sqrtf(x_2 + y_2 + z_2);
...
void NavMeshTesterTool::addAgent(const float s_pos[], const float e_pos[], float r, float h, float c, const unsigned int col)
{
	m_sposSet = true;
	m_spos[0] = s_pos[0];
	m_spos[1] = s_pos[1];
	...
	m_epos[2] = e_pos[2];
	recalc();
	// this->checkQueryStatus(this->m_pathFindStatus);
}

std::vector<float> NavMeshTesterTool::getPath()
{
	std::vector<float> points;
	for (int i = 0; i < m_nstraightPath; ++i)
	{
		float x=0, y=0, z=0;
		x = (m_straightPath[(i)*3]) ;
		y = (m_straightPath[(i)*3+1]);
		z = (m_straightPath[(i)*3+2]);
		// dd.vertex(m_smoothPath[i*3], m_smoothPath[i*3+1]+0.1f, m_smoothPath[i*3+2], spathCol);
		points.push_back(x);
		points.push_back(y);
		points.push_back(z);
...
std::vector<float> NavMeshTesterTool::getFollowPath()
{
	std::vector<float> points;
	for (int i = 0; i < m_nsmoothPath; ++i)
	{
		float x=0, y=0, z=0;
		x = (m_smoothPath[(i)*3]) ;
		y = (m_smoothPath[(i)*3+1]);
		z = (m_smoothPath[(i)*3+2]);
		// dd.vertex(m_smoothPath[i*3], m_smoothPath[i*3+1]+0.1f, m_smoothPath[i*3+2], spathCol);
		points.push_back(x);
		points.push_back(y);
		points.push_back(z);
...
void NavMeshTesterTool::setToolMode(ToolMode mode)
{
	this->m_toolMode=mode;
}
