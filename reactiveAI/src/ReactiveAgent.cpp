...
#include "SteerLib.h"
#include "ReactiveAIModule.h"
#include "ReactiveAgent.h"
#include <math.h>

using namespace Util;
using namespace SteerLib;
using namespace ReactiveGlobals;
...
// reset()
//
void ReactiveAgent::reset(const SteerLib::AgentInitialConditions & initialConditions, SteerLib::EngineInterface * engineInfo)
{
	...
	_mass = 1.0f;
	_maxSpeed = ped_max_speed; //  PED_MAX_SPEED;
	...
	__oldPositions = std::vector<Util::Point>();
#endif
...
// addGoal()
//
void ReactiveAgent::addGoal(const SteerLib::AgentGoalInfo & newGoal) {
...
// updateAI()
//
void ReactiveAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{


	if (!_enabled) return;

	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->aiProfiler );
...
// runCognitivePhase()
//
void ReactiveAgent::runCognitivePhase()
{
...
// runLongTermPlanningPhase()
//
void ReactiveAgent::runLongTermPlanningPhase()
{
	...
	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->longTermPhaseProfiler );
...
// runMidTermPlanningPhase()
//
void ReactiveAgent::runMidTermPlanningPhase()
{
	...
	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->midTermPhaseProfiler );
	...
	if (_midTermPathSize > ped_next_waypoint_distance + 3) {
		...
		std::cerr << "ERROR!!!  _midTermPathSize is larger than expected: should be less than or equal to " << ped_next_waypoint_distance+3 << ", but it actually is " << _midTermPathSize << "\n";
...
// runShortTermPlanningPhase()
//
void ReactiveAgent::runShortTermPlanningPhase()
{
	...
	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->shortTermPhaseProfiler );
#ifdef _DEBUG
	std::cout << "about to accessgSpatialDatabase1\n";
#endif
	int myIndexPosition = gSpatialDatabase->getCellIndexFromLocation(_position.x, _position.z);
...
#ifdef _DEBUG
	std::cout << "about to (closestPathNode != _midTermPathSize-1)\n";
#endif
...
#ifdef _DEBUG
	std::cout << "about to (closestPathNode < _midTermPathSize-3)\n";
#endif
...
#ifdef _DEBUG
	std::cout << "done (localTargetIndex <= furthestTargetIndex) \n";
#endif
...
#ifdef _DEBUG
	std::cout << "done localTargetIndex--" << localTargetIndex << " \n";
#endif
			if (localTargetIndex >= closestPathNode)
			{
				...
				gSpatialDatabase->getLocationFromIndex( localTargetCellID, _localTargetLocation );
#ifdef _DEBUG
	std::cout << "done gSpatialDatabase->getLocationFromIndex( localTargetCellID, _localTargetLocation );\n";
#endif
...
#ifdef _DEBUG
	std::cout << "done localTargetCellID = _midTermPath[localTargetIndex];\n";
#endif
			}
			else {
				// if localTargetIndex is pointing backwards, then just aim for 2 nodes ahead of the current closestPathNode.
#ifdef _DEBUG
	std::cout << "about to localTargetCellID = _midTermPath[closestPathNode+2];\n";
#endif
				localTargetCellID = _midTermPath[closestPathNode+2];
#ifdef _DEBUG
	std::cout << "done localTargetCellID = _midTermPath[closestPathNode+2];\n";
#endif
				gSpatialDatabase->getLocationFromIndex( localTargetCellID, _localTargetLocation );
			}
		}
...
#ifdef _DEBUG
	std::cout << "about to gSpatialDatabase->getLocationFromIndex( closestPathNode, _localTargetLocation);\n";
#endif
...
#ifdef _DEBUG
	std::cout << "about to (gUseDynamicPhaseScheduling)1\n";
#endif

	if (gUseDynamicPhaseScheduling) {
#ifdef _DEBUG
	std::cout << "about to (gUseDynamicPhaseScheduling)2\n";
#endif
...
// runPerceptivePhase()
//
void ReactiveAgent::runPerceptivePhase()
{
	if (!_enabled) return;

	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->perceptivePhaseProfiler );
...
// runPredictivePhase()
//
void ReactiveAgent::runPredictivePhase()
{
	if (!_enabled) return;

	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->predictivePhaseProfiler );
	...
	if (_steeringState != STEERING_STATE_TURN_TOWARDS_TARGET) {	// ignore threats in the STEERING_STATE_TURN_TOWARDS_TARGET state.
		...
		//for (unsigned int i=0; i<_neighbors.size(); i++) {
			...
			ReactiveAgent * otherGuy = dynamic_cast<ReactiveAgent*>(*neighbor);
...
// runReactivePhase()
//
void ReactiveAgent::runReactivePhase()
{
	if (!_enabled) return;
	...
	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->reactivePhaseProfiler );
...
*/
	...
	if (hitSomething) {
		...
		if (   ((feelers.object_front) && (feelers.object_front->isAgent()) && (!threatListContainsAgent(dynamic_cast<ReactiveAgent*>(feelers.object_front))))
			|| ((feelers.object_left) && (feelers.object_left->isAgent()) && (!threatListContainsAgent(dynamic_cast<ReactiveAgent*>(feelers.object_left))))
			|| ((feelers.object_right) && (feelers.object_right->isAgent()) && (!threatListContainsAgent(dynamic_cast<ReactiveAgent*>(feelers.object_right)))))
		{
			foundNewThreat = true;
		}
		...
		if (   ((feelers.object_front) && (feelers.object_front->isAgent()) && (threatListContainsAgent(dynamic_cast<ReactiveAgent*>(feelers.object_front), tempIndex)) && (!_threatList[tempIndex].imminent) )
			|| ((feelers.object_left) && (feelers.object_left->isAgent()) && (threatListContainsAgent(dynamic_cast<ReactiveAgent*>(feelers.object_left), tempIndex)) && (!_threatList[tempIndex].imminent) )
			|| ((feelers.object_right) && (feelers.object_right->isAgent()) && (threatListContainsAgent(dynamic_cast<ReactiveAgent*>(feelers.object_right), tempIndex)) && (!_threatList[tempIndex].imminent) ))
		{
		...
		if ((feelers.object_front) && (feelers.object_front->isAgent())) {
			numAgentsHit++;
			ReactiveAgent * p = dynamic_cast<ReactiveAgent*>(feelers.object_front);
		...
		if ((feelers.object_left) && (feelers.object_left!=feelers.object_front) && (feelers.object_left->isAgent())) {
			numAgentsHit++;
			ReactiveAgent * p = dynamic_cast<ReactiveAgent*>(feelers.object_left);
			Vector dV = _velocity - p->velocity();
			Vector dO = _position - p->position();
			float distanceThreshold = _radius + p->radius() + ped_dynamic_collision_padding;
			float A = dot(dV,dV);
		...
		if ((feelers.object_right) && (feelers.object_right!=feelers.object_front) && (feelers.object_right!=feelers.object_left) && (feelers.object_right->isAgent())) {
			numAgentsHit++;
			ReactiveAgent * p = dynamic_cast<ReactiveAgent*>(feelers.object_right);
			Vector dV = _velocity - p->velocity();
			Vector dO = _position - p->position();
			float distanceThreshold = _radius + p->radius() + ped_dynamic_collision_padding;
			float A = dot(dV,dV);
		...
		if (_steeringState == STEERING_STATE_TURN_TOWARDS_TARGET) {
			...
			if ((feelers.object_left)&&(feelers.object_left->isAgent())) {
				float tempVelocity = dot(forward(),(dynamic_cast<ReactiveAgent*>(feelers.object_left))->velocity());
			...
			if ((feelers.object_right)&&(feelers.object_right->isAgent())) {
				float tempVelocity = dot(forward(),(dynamic_cast<ReactiveAgent*>(feelers.object_right))->velocity());
				//if (tempVelocity > -1.0f)
					_finalSteeringCommand.targetSpeed = min(_finalSteeringCommand.targetSpeed,tempVelocity);
			}
			if ((feelers.object_front)&&(feelers.object_front->isAgent())) {
				float tempVelocity = dot(forward(),(dynamic_cast<ReactiveAgent*>(feelers.object_front))->velocity());
				//if (tempVelocity > -1.0f)
					_finalSteeringCommand.targetSpeed = min(_finalSteeringCommand.targetSpeed,tempVelocity);
			}
			...
			assert(obj!=NULL);
			ReactiveAgent * p = dynamic_cast<ReactiveAgent*>(obj);
			...
			if ( cosTheta < 0.0f ) {
				...
				if (_threatList[_mostImminentThreatIndex].threatType == PredictedThreat::THREAT_TYPE_ONCOMING) {
					...
					ReactiveAgent * threatGuy = _threatList[_mostImminentThreatIndex].threatGuy;
			...
			if (reactiveSituation == REACTIVE_SITUATION_ONE_AGENT) {
				...
				if ( cosTheta < ped_oncoming_reaction_threshold ) {
					...
					if (his_time < my_time) {
						...
						_finalSteeringCommand.targetSpeed = min(_finalSteeringCommand.targetSpeed, (float)ped_slower_speed_factor * tempVelocity);
					}
					else {
				...
				ReactiveAgent * pLeft = dynamic_cast<ReactiveAgent*>(objLeft);
				ReactiveAgent * pRight = dynamic_cast<ReactiveAgent*>(objRight);
				...
				ReactiveAgent * p = dynamic_cast<ReactiveAgent*>(objAgent);
				...
				// unlikely case...
				//ReactiveAgent * pLeft = dynamic_cast<ReactiveAgent*>(feelers.object_left);
				//ReactiveAgent * pMiddle = dynamic_cast<ReactiveAgent*>(feelers.object_front);
				//ReactiveAgent * pRight = dynamic_cast<ReactiveAgent*>(feelers.object_right);
	...
	//========================
		...
		if (_threatList[_mostImminentThreatIndex].threatType == PredictedThreat::THREAT_TYPE_ONCOMING) {
			...
			ReactiveAgent * threatGuy = _threatList[_mostImminentThreatIndex].threatGuy;//((Pedestrian*)(imminentThreat->object));
			if ( dot((_rightSide),threatGuy->forward()) < 0.0f) {
				_finalSteeringCommand.aimForTargetSpeed = true;
				_finalSteeringCommand.targetSpeed = ped_slightly_faster_speed_factor*_currentGoal.desiredSpeed;
				_finalSteeringCommand.aimForTargetDirection = false;
...
// doSteering()
//
void ReactiveAgent::doSteering()
{
	if (!_enabled) return;

	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->steeringPhaseProfiler );
...
//
void ReactiveAgent::doEulerStepWithForce(const Vector & force)
{
...
// doDynamicsSteering()
//
void ReactiveAgent::doDynamicsSteering()
{
...
// doCommandBasedSteering()
//
void ReactiveAgent::doCommandBasedSteering()
{
...
// collectObjectsInVisualField()
//
void ReactiveAgent::collectObjectsInVisualField()
{
	_neighbors.clear();
...
// reachedCurrentGoal()
//
bool ReactiveAgent::reachedCurrentGoal()
{
...
//
bool ReactiveAgent::reachedCurrentWaypoint()
{
...
// reachedLocalTarget()
//
bool ReactiveAgent::reachedLocalTarget()
{
	return ( (_localTargetLocation-_position).lengthSquared() < (ped_reached_target_distance_threshold * ped_reached_target_distance_threshold) );
}
...
//
bool ReactiveAgent::threatListContainsAgent(ReactiveAgent * agent, unsigned int & index)
{
	for (unsigned int i=0; i < _threatList.size(); i++) {
...
//
bool ReactiveAgent::updateReactiveFeelers( FeelerInfo & feelers )
{
	...
	myRay.initWithLengthInterval(_position, _forward * (ped_typical_speed*ped_reactive_anticipation_factor) * 1.1f);
	myRightRay.initWithLengthInterval( _position + _radius * _rightSide ,  ((_forward * 0.75f) + 0.1f*_rightSide)* (ped_typical_speed*ped_reactive_anticipation_factor));
	myLeftRay.initWithLengthInterval( _position - _radius * _rightSide,  ((_forward * 0.75f) - 0.1f*_rightSide)* (ped_typical_speed*ped_reactive_anticipation_factor));
...
// drawPlannedPath()
//
void ReactiveAgent::drawPlannedPath()
{
	// longTermPath.empty();
...
#else
#ifdef _DEBUG2
	// std::cout << "longtermPath size: " << longTermPath.size() << "\n";
#endif
	if (longTermPath.size() > 0)
	{
...
// draw()
//
void ReactiveAgent::draw()
{
	// DrawLib::drawAgent
#ifdef ENABLE_GUI
	if (!_enabled) return;
	AutomaticFunctionProfiler profileThisFunction( &ReactiveGlobals::gPhaseProfilers->drawProfiler );
	...
	if ( this->isSelected() )
		...
		DrawLib::drawAgentDisc(_position, _forward, _radius, Util::gDarkGreen);
	...
	{
		DrawLib::drawLine(__oldPositions.at(q), __oldPositions.at(q+1),gBlack);
	}
#endif
	...
	*/
	gSpatialDatabase->getItemsInRange(_neighbors, this->position().x-(this->_radius * 3), this->position().x+(this->_radius * 3),
			this->position().z-(this->_radius * 3), this->position().z+(this->_radius * 3), dynamic_cast<SteerLib::SpatialDatabaseItemPtr>(this));
	...
	// DrawLib::drawLine(this->position(), this->position()+(__plannedSteeringForce), gGreen);
#else
	/*
	__oldPositions.push_back(position());
	for (int q = 0; q < __oldPositions.size() -2 && __oldPositions.size() > 1;q++ )
